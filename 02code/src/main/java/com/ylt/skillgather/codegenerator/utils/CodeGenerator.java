package com.ylt.skillgather.codegenerator.utils;
...
public class CodeGenerator {
   ...
   /* public static void main(String[] args) {
    ...
    public static boolean generate(CodeMoudlEntity codeMoudlEntity) {
        ...
        strategy.setSuperControllerClass("com.ylt.skillgather.coreframe.controller.BaseController");//设置controler集成父类

        String[] tableStrings=new String[codeMoudlEntity.items.size()];
        for (int i = 0; i < codeMoudlEntity.items.size(); i++) {
            tableStrings[i]=codeMoudlEntity.items.get(i).TableName;
        }
        strategy.setInclude(tableStrings);
        ...
        pc.setModuleName(codeMoudlEntity.mouldname);
        //pc.setEntity();//默认已经设置
     ...
     /*   tc.setEntity(null);
        ...
        InjectionConfig cfg = new InjectionConfig() {
            ...
            public void initMap() {
                ...
                map.put("menu", codeMoudlEntity.menuname);
                this.setMap(map);
            }
        };
        // 自定义模板生成，可以生成各层级别的文件，但主要是生成html（增删改查），以下生成查看为例
        List<FileOutConfig> focList = new ArrayList<>();
        focList.add(new FileOutConfig("/vm/templateview.html.vm") {
            @Override
